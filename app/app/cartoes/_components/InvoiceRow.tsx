    ...
    <li className="space-y-2 rounded-xl border border-border p-3">
      ...
      {allEntries.length > 0 && (
        <ul className="divide-y divide-border rounded-lg border border-border text-xs">
          {allEntries.map((t) => (
            <li key={t.id} className="flex flex-col gap-1.5 px-3 py-2 sm:flex-row sm:items-center sm:gap-3 sm:py-1.5">
              {/* Linha 1 mobile (full row no desktop): data + merchant + valor + void */}
              ...
              {/* Linha 2 mobile (no desktop entra entre merchant e valor): badges */}
...
function Badges({ entry: t }: { entry: Invoice["lumpSumEntries"][number] }) {
  return (
    <>
      {t.categoryLabel ? (
