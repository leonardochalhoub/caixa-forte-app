...
export default async function SysadminPage() {
  ...
  // Short pt-BR explanations generated by Groq from the monthly numbers
  // themselves. Cheap enough to run on every admin render (one request per
  // page load, all three periods in one JSON completion).
  const trendMonthly = trend.map((t) => ({ month: t.month, netCents: t.net }))
  const explanations = await explainTrends(
    {
      label: "mês atual",
      direction: trend1m.direction,
      netCents: trend1m.net,
      monthly: trendMonthly.slice(0, 1),
    },
    {
      label: "últimos 6 meses",
      direction: trend6m.direction,
      netCents: trend6m.net,
      monthly: trendMonthly.slice(0, 6),
    },
    {
      label: "últimos 12 meses",
      direction: trend12m.direction,
      netCents: trend12m.net,
  ...
  const kpi = {
    ...
    trend1m: { ...trend1m, why: explanations.current },
    trend6m: { ...trend6m, why: explanations.last6 },
    trend12m: { ...trend12m, why: explanations.last12 },
  }
