-- Caixa Forte — pay_invoice agora linka transfer_peer_id nos pares criados
-- Sequência: 0035 cria pay_invoice; 0037 adiciona tx_kind nos inserts;
-- 0038 cria transfer_peer_id; 0039 atualiza pay_invoice pra setar.
...
begin
  ...
  insert into public.transactions
    (user_id, account_id, type, amount_cents, occurred_on, merchant, note,
     source, is_transfer, paid_at, tx_kind, transfer_peer_id)
  values
    (v_user_id, p_card_id, 'income', p_amount_cents, v_today,
     v_merchant, 'Entrada crédito — fatura ' || p_invoice_label || ' paga',
     'manual', true, v_now, 'invoice_payment', v_expense_id)
  returning id into v_income_id;

  -- Linka o expense de volta pro income (bidirecional)
